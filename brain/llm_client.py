    ...
    def _build_prompt(self, prompt_name: str, variables: dict) -> str:
        ...
        prompt_blocks = {
            "turn_interpreter": (
                ...
                "absent 表示患者本轮表达支持该项不存在、结果明确阴性、或在询问检查 availability 时明确未做过；"
                "unclear 表示患者本轮无法明确支持或否定该项。"
                "如果 question_type 是 lab、imaging、pathogen，或 acquisition_mode / evidence_cost 表示高成本检查，"
                "患者回答“没做过这项检查”“没听医生提过”“报告里没注意到”“不记得有没有”时，"
                "只能把 pending_target_name 标为 unclear，不能标为 absent；"
                "只有患者明确说检查结果阴性、报告没有提示、医生明确排除或确切否认该项时，才标为 absent。"
                "如果 relation_type=DIAGNOSED_BY 或 pending_target_label 属于 LabFinding、LabTest、ImagingFinding、Pathogen，"
                "同样要区分“未检查/没听说”(unclear) 和“结果明确阴性/不存在”(absent)。"
                "即使 patient_text 里出现“没有/没/未”等否定词，也必须先判断它否定的是“做过检查/听说过结果”，"
                "还是否定目标证据本身；前者是信息缺失，后者才是 absent。"
                ...
                "示例3：问“最近有没有做过胸部CT？”，答“胸部CT没做过”，则 mentions 可包含 {name:\"胸部CT\", polarity:\"absent\"}。"
                "示例4：问“头颅CT有没有低密度病灶？”，答“没做过这项检查”，则 mentions 应包含 {name:\"头颅CT低密度病灶\", polarity:\"unclear\"}。"
                "示例5：问“有没有检出某个病原体？”，答“没听医生提过”，则 mentions 应包含 {name: pending_target_name, polarity:\"unclear\"}。"
            ...
            "patient_opening_generation": (
                ...
                "如果 opening_slots 以检查项为主，必须保留槽位名称里的关键医学锚点："
                "数值/阈值/单位（如低于200、<200）、阳性/阴性/升高/降低、具体病原体名或影像异常名；"
                "不得把 CD4 < 200、HIV RNA阳性、具体病原体名等压缩成单纯“异常/偏低”。"
                "如果以症状为主，要优先用症状组织表达。"
            ...
            "patient_slot_semantic_match": (
                ...
                "如果 question_text 问的是普通症状、低成本病史或风险因素，可以让 no_match_answer 填一句简短明确否定，"
                "例如“没有这个症状。”。"
                "如果 question_text 问的是 lab/imaging/pathogen、高成本检查、病原体、检查结果或疾病定义性证据，"
                "no_match_answer 应表达未检查、没听医生提过、报告里没注意到或不确定，"
                "例如“没做过这项检查。”、“没听医生提过这个结果。”、“报告里没注意到。”；"
                "这些回答会被 brain 解析为 unclear，表示病例骨架未提供该项，而不是检查结果阴性；"
                "不要写成“没有相关情况”或明确阴性，除非病例候选中存在明确阴性槽位。"
            ),
            "baseline_consultation_turn": (
                "你现在扮演一位多轮问诊医生。"
                "你的任务是在给定对话历史、已问问题和剩余预算的前提下，"
                "每一轮只做一个决策：继续 ask 一个问题，或直接 final 给出最终判断。"
                "必须严格输出 JSON object，且只允许包含以下字段："
                "decision、question_text、target_name、top3、reasoning、compiled、final_answer。"
                "decision 只能精确取 ask 或 final。"
                "如果 must_finalize=true，必须输出 decision=final，不能继续 ask。"
                "如果输入里提供了 retrieved_documents，"
                "它们表示从外部语料中检索到的参考资料，可辅助你判断下一问或最终答案；"
                "但不要把 retrieved_documents 中的内容误当成患者已经明确确认过的事实。"
                "如果输入里提供了 retrieved_kg_context，"
                "它们表示从当前活跃搜索图谱整理出的候选疾病相关证据块，也只能作为参考；"
                "同样不要把 retrieved_kg_context 中的内容直接当成患者已经确认的事实。"
                "如果输入里提供了 retrieved_kg_candidate_diseases，"
                "它们表示基于当前已知症状从图谱反查出的候选疾病列表，可用于补充鉴别诊断；"
                "这些候选同样只是检索参考，不等于患者已经确诊，也不要仅因重复出现就机械抬高其置信度。"
                "question_text 是面向患者的中文问题，只在 decision=ask 时填写；"
                "必须是单一目标、单一意图的问题，不允许一次问多个 target，"
                "也不允许“有没有 A/B/C”这种并列三合一问法。"
                "target_name 表示这条问题真正要验证的单一目标，例如“发热”“HIV 暴露史”“实验室检查”“胸部CT磨玻璃影”“结核分枝杆菌PCR”。"
                "系统会根据 question_text 和 target_name 在程序端推断问题分组与证据成本，"
                "因此你不需要额外输出 question_group、evidence_cost 或顶层 confidence。"
                "top3 必须始终是数组，表示你当前最可能的 3 个诊断候选，按置信度从高到低排序；"
                "每个元素都必须是对象，且只允许包含 name、confidence 两个字段。"
                "如果输入里提供了 disease_scope_names，"
                "则 top3 里的每个 name 以及 final_answer 都必须从这份 disease_scope_names 中精确选择；"
                "不要输出范围外病名，不要输出上位类、组合类、泛化描述或“某类机会性感染”这类非标准答案。"
                "reasoning 必须是简短中文，说明为什么此时选择 ask 或 final。"
                "compiled 只在 decision=final 时有意义，表示你已经愿意收束为最终判断；"
                "若 decision=final，应置为 true。"
                "final_answer 只在 decision=final 时填写，必须是 top3 的第 1 名。"
                "如果当前证据仍明显不足，优先 ask；"
                "但如果剩余预算耗尽、must_finalize=true，或继续追问价值已经很低，应改为 final。"
                "你可以参考 asked_questions 避免重复追问。"
            ),
            ...
            "exam_context_interpretation": (
                ...
                "如果当前问题是在问某个具体检查结果或病原体是否阳性，而患者只说“没听医生提过这个结果”"
                "或“报告里没注意到”，这不等于结果阴性；availability 可按上下文为 unknown 或 done，"
                "mentioned_results 不要编造 negative 结果。"
            ...
            "trajectory_agent_verifier": (
                ...
                "给出该答案的代理评审分数。"
                "你必须严格区分 observed_session_evidence 与 simulated_trajectory_evidence："
                "observed_session_evidence 是真实会话中患者已经确认或否认的证据；"
                "simulated_trajectory_evidence / best_trajectory 是系统 rollout 假设出来的验证路径，"
                "只能用于判断下一步值得问什么，不能当作患者真实已经做过或真实阳性的检查结果。"
                "只有 observed_session_evidence 或患者累计上下文中已有候选答案的关键支持证据时，"
                "才允许因为 key_support_sufficient 输出 should_accept_stop=true；"
                "如果关键病原学、影像、培养、PCR、抗体等证据只出现在 best_trajectory 的模拟阳性分支，"
                "必须输出 should_accept_stop=false、reject_reason=missing_key_support，"
                "并把这些模拟阳性证据放入 recommended_next_evidence 作为下一轮真实追问目标。"
    ...
    def _build_verifier_acceptance_profile_prompt(self, profile: str) -> str:
        ...
        if normalized in {"guarded_lenient", "guarded-lenient", "guarded"}:
            return (
                ...
                "但 simulated_trajectory_evidence 中的阳性检查不算 confirmed evidence；"
                "如果关键支持只来自 rollout 模拟路径，而 observed_session_evidence 未确认，必须拒停并建议真实验证。"
