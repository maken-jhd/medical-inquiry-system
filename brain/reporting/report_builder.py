...
from ..state import FinalAnswerScore, ReasoningTrajectory, SearchResult, SessionState, StopDecision
...
class ReportBuilder:
    ...
    def build_final_report(self, session_state: SessionState, stop_decision: StopDecision) -> Dict[str, Any]:
        # final_report 是最基础的阶段性输出，不依赖 search_result 也能单独成立。
    ...
    def build_search_report(
        ...
        # search_report 主要服务调试和 replay，保留 root action、repair 和 answer score 等中间观测。
        return {
            "session_id": session_state.session_id,
            "turn_index": session_state.turn_index,
    ...
    def build_final_reasoning_report(
        ...
        # reasoning report 是在 final_report 基础上追加“为什么停、为什么是这个答案”的解释层。
