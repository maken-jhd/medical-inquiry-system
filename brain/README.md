...
- [llm_client.py](/Users/loki/Workspace/GraduationDesign/brain/llm_client.py)
  - 统一封装第二阶段大模型结构化调用。
  - 当前供 `MedExtractor`、`A1` 抽取和 `A2` 假设排序等模块复用。
...
- [retriever.py](/Users/loki/Workspace/GraduationDesign/brain/retriever.py)
  ...
  - 当前已经实现论文风格的 `R1 / R2` 双向检索基础版，并补充了真实 Neo4j smoke 检查入口。
...
- [service.py](/Users/loki/Workspace/GraduationDesign/brain/service.py)
  - 第二阶段的总编排层。
  - 当前已经串联 `PatientContext -> A1 -> A2 -> R2/A3 -> rollout -> report` 的最小搜索闭环。
...
- A1-A4 的第一批模块已建立
- UCT、局部 simulation 和轨迹评分都已有最小可运行版本
- `service.py` 已经能够跑通最小搜索闭环
- 但还没有完全复现论文中的深层 rollout、完整 verifier 和最终轨迹判别器
...
| A3 | 部分完成 | 已支持 R2 检索、动作构造与问题生成 |
| A4 | 部分完成 | 已支持目标感知解释和显式路由 |
| R1 / R2 | 基础版完成 | 已与真实 Neo4j 联调 |
| Search Tree | 基础版完成 | 已有显式树和回传统计 |
| Rollout | 浅层版完成 | 已支持局部轨迹输出 |
| Path Evaluation | 基础版完成 | 已支持一致性 / 多样性 / agent score |
