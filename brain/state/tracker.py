...
from .results import ReasoningTrajectory, SearchResult
from .runtime import (
    ActionStats,
    ExamAvailability,
    ExamContextState,
...
class StateTracker:
    ...
    def create_session(self, session_id: str) -> SessionState:
        # 会话初始化时只建立最小空状态，后续所有阶段都在这份对象上增量写入。
    ...
    def get_session(self, session_id: str) -> SessionState:
        # tracker 是 session_state 的唯一真相来源，外层不应自行缓存第二份可写状态。
    ...
    def build_rollout_session_snapshot(state: SessionState) -> SessionState:
        # rollout snapshot 刻意丢掉 search_tree、trajectory 历史和统计缓存，避免模拟分支污染真实会话。
        return SessionState(
    ...
    def apply_slot_updates(self, session_id: str, updates: Iterable[SlotUpdate]) -> SessionState:
        # 槽位批量写回是 turn merge、exam_context merge 和 rollout update 的通用收口点。
    ...
    def merge_mention_items(
        ...
        *,
        ...
        # mention_context 保存“患者说过什么”的会话级记忆，后面 trusted evidence 和重复提问过滤都会用到。
        state = self.get_session(session_id)
    ...
    def set_candidate_hypotheses(
        ...
        # A2、observed anchor rerank 和 verifier repair 都通过这里统一覆盖当前候选排序。
        state = self.get_session(session_id)
    ...
    def save_trajectory(self, session_id: str, trajectory: ReasoningTrajectory) -> None:
        # rollout 产出的轨迹先落到 session_state，方便 search report、final report 和离线复盘统一读取。
        state = self.get_session(session_id)
    ...
    def mark_question_asked(self, session_id: str, node_id: str) -> None:
        # asked_node_ids 是 R2 过滤、repeat guard 和 root action 选择的重要共享状态。
        state = self.get_session(session_id)
    ...
    def activate_topic(self, session_id: str, topic_id: str) -> None:
        # topic 反映当前问诊还围绕哪个 hypothesis / 证据簇继续深入。
        state = self.get_session(session_id)
    ...
    def increment_turn(self, session_id: str) -> int:
        # turn_index 在单轮入口最先推进，保证本轮新增 evidence / mention 都能记录正确来源轮次。
        state = self.get_session(session_id)
        state.turn_index += 1
    ...
    def set_evidence_state(self, session_id: str, evidence_state: EvidenceState) -> EvidenceState:
        # evidence_state 比普通 slot 更贴近图谱节点，主要服务 hypothesis feedback、R2 过滤与 verifier。
        state = self.get_session(session_id)
    ...
    def get_pending_action(self, session_id: str) -> Optional[MctsAction]:
        # pending_action 是多轮闭环的关键，它告诉下一轮“患者这句话大概率在回答什么问题”。
        state = self.get_session(session_id)
    ...
    def set_pending_action(self, session_id: str, action: MctsAction) -> None:
        # 下一轮 turn_interpreter 和 pending_action merge 都会先从这里拿上一轮动作上下文。
        state = self.get_session(session_id)
    ...
    def clear_pending_action(self, session_id: str) -> None:
        # 一旦本轮回答已经被消化，就要及时清掉 pending，避免后续自由描述被误判成继续回答旧问题。
        state = self.get_session(session_id)
    ...
    def increment_state_visit(
        ...
        # state visit 统计主要给树搜索复用，用来解释“某个 belief state 被 rollout 访问了多少次”。
        state = self.get_session(session_id)
