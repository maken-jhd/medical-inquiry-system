...
from ..shared.errors import LlmEmptyExtractionError, LlmOutputInvalidError, LlmUnavailableError
from ..integrations.llm import LlmClient
from ..shared.normalization import NameNormalizer
from ..state import (
    A1ExtractionResult,
    ClinicalFeatureItem,
    ExamContextResult,
...
class EvidenceParser:
    ...
    def interpret_turn(
        ...
        # 统一解释入口先尝试“直接回答规则”，只有规则不够用时才进入 LLM 结构化抽取。
    ...
    def run_a1_key_symptom_extraction(
        ...
        # A1 尽量复用当前轮已经解释好的 mentions，避免为同一句患者输入重复再跑一套抽取链。
    ...
    def derive_pending_action_result_from_text(
        ...
        # 旧接口若只给原始文本，也统一先转成 turn_result，再走同一套 pending_action 解释逻辑。
    ...
    def interpret_exam_context_answer(
        ...
        # exam_context 先判断“做过/没做过/不确定”这类直答，再决定是否需要 LLM 解析检查名称与结果。
    ...
    def build_patient_context_from_turn(
        ...
        # PatientContext 是 turn 层给 A1/A2/A3/verifier 的兼容视图，核心仍然来自统一 mentions。
