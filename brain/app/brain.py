...
from ..search.action_builder import ActionBuilder, ActionBuilderConfig
from ..integrations.entity_linker import EntityLinker, EntityLinkerConfig
from ..acceptance import AcceptanceCalibrationConfig, AcceptanceCoordinator, VerifierAcceptanceController
from ..state.anchors import EvidenceAnchorAnalyzer, EvidenceAnchorConfig
from ..turn.parser import EvidenceParser, EvidenceParserConfig
from ..shared.errors import LlmEmptyExtractionError, LlmUnavailableError
from ..search.hypothesis_manager import HypothesisManager, HypothesisManagerConfig
from ..integrations.llm import LlmClient
from ..search.mcts import MctsConfig, MctsEngine
from ..turn.extractor import MedExtractor
from ..integrations.neo4j import Neo4jClient
from ..search.fallback import QuestionSelector
from ..reporting.report_builder import ReportBuilder
from ..search.transition_model import (
    HeuristicResponseTransitionModel,
    ResponseTransitionModelConfig,
    StatisticalResponseTransitionModel,
)
from ..search.reward_model import HeuristicRolloutRewardModel, RolloutRewardModelConfig
from ..search.reward_model import BeliefAwareRolloutRewardModel
from ..search.retriever import GraphRetriever, RetrievalConfig
from ..search.router import ReasoningRouter, RouterConfig
from ..search.tree import SearchTree
from ..search.simulation import SimulationConfig, SimulationEngine
from ..state.signature import BeliefStateSignatureBuilder, StateSignatureConfig
from ..state.tracker import StateTracker
from ..search.transition_statistics import TransitionStatisticsBuilder, TransitionStatisticsConfig
from ..search import SearchCoordinator
from ..search.evaluator import TrajectoryEvaluator, TrajectoryEvaluatorConfig
from ..state import (
    A1ExtractionResult,
    A2HypothesisResult,
...
class BrainRuntime:
    ...
    def __init__(self, deps: BrainDependencies) -> None:
        # 依赖装配只在这里补默认值；后续所有 coordinator/runtime 逻辑都假定这些组件已经齐备。
    ...
    def finalize(self, session_id: str) -> dict:
        ...
        # 如果当前会话还没形成可复用的 search 结果，就退回普通 final_report 兜底。
    ...
    def _prepare_turn_mentions(
        ...
        # 先把统一提及项对齐到 KG 实体；后面 A2、trusted evidence 与解释展示都复用这批 graph-grounded mentions。
        ...
        for mention, linked in zip(turn_result.mentions, linked_results):
            ...
            if pending_action is not None and mention.node_id is None:
                # 如果 entity linker 没命中，但这句明显在回答上一轮目标节点，就补上 pending_action 的 target_node_id。
    ...
    def update_from_pending_action(
        ...
        # 这一步负责回答一个核心问题：
        # 当前患者输入是不是在回答上一轮问题，以及回答后要怎样写回状态并切到下一阶段。
        ...
        evidence_state.metadata.update(
            {
                ...
                "confirmed_family_candidates": sorted(evidence_tags & CONFIRMED_EVIDENCE_ROLE_TAGS),
                "provisional_family_candidate": provisional_family_candidate,
                "provisional_family_candidates": sorted(
                    evidence_tags & PROVISIONAL_EVIDENCE_ROLE_TAGS
                ),
    ...
    def _run_a2(
        ...
        # A2 先聚合首轮症状、实体链接和 anchor rescue 候选，再统一交给 hypothesis_manager 做主备选排序。
        tracker = self.deps.state_tracker
    ...
    def _run_reasoning_search_impl(
        self,
        session_id: str,
        patient_context: PatientContext,
    ) -> SearchResult:
        # no-tree greedy 是最轻量的实验路径：完全跳过树搜索，只看当前候选态的动作排序。
        ...
        # 常规搜索在进入树之前，先把“患者已经自己说过的图谱证据”物化成 evidence_state。
    ...
    # 将搜索结果转成一条可直接用于提问的动作。
    def _choose_next_question_from_search_impl(self, session_id: str, search_result: SearchResult) -> MctsAction | None:
        # 若搜索已明确给出 selected_action，就直接沿用；否则才退回全局冷启动问题。
    ...
    def _finalize_from_search_impl(
        self,
        session_id: str,
        search_result: SearchResult,
    ) -> dict:
        # 最终报告只消费 search 聚合后的答案评分与 acceptance 结果，不在这里重复跑检索或 rollout。
        state = self.deps.state_tracker.get_session(session_id)
        best_answer_score = self.deps.trajectory_evaluator.select_best_answer(search_result.final_answer_scores)
        accept_decision = self.deps.acceptance_controller.should_accept_final_answer(best_answer_score, state)
        return self.deps.report_builder.build_final_reasoning_report(state, accept_decision, search_result)
    ...
    def _process_turn_impl(self, session_id: str, patient_text: str) -> dict:
        ...
        # 如果已经追问过一次主诉，但本轮仍无任何有效线索，则停止重复 intake，避免空转。
            ...
            if effective_stage in {"A2", "A3"} and len(tracker.get_session(session_id).candidate_hypotheses) > 0:
                ...
                search_result = self._run_reasoning_search_impl(session_id, patient_context)
                default_search_action = self._choose_next_question_from_search_impl(session_id, search_result)
        ...
        if self._should_emit_final_report(search_result, selected_action, stop_decision, accept_decision):
            if stop_decision.reason in {
                ...
                final_report = (
                    self._finalize_from_search_impl(session_id, search_result)
                    if self._has_search_signal(search_result)
    ...
    # 根据动作文本补充通用证据 family，避免完全依赖图谱边上的标签。
    def _anchor_action_families_from_text(self, normalized_text: str) -> set[str]:
        anchor_rules: list[tuple[set[str], tuple[str, ...]]] = [
            ...
            ({"pathogen", "disease_specific_lab"}, ("βd葡聚糖", "bdg", "葡聚糖", "g试验")),
            ({"pathogen", "disease_specific_lab"}, ("pcr", "核酸", "病原", "支气管肺泡", "balf", "bal")),
            ({"tuberculosis"}, ("tspot", "tspot.tb", "t-spot", "xpert", "mtb/rif", "抗酸", "分枝杆菌")),
            ({"metabolic_definition"}, ("ldl", "hdl", "甘油三酯", "总胆固醇", "胆固醇", "血脂")),
        ]
    ...
    # 判断当前上一轮动作结果是否具备被 anchor / repair 计入 confirmed family 的基础条件。
    def _is_confirmed_family_candidate(
        ...
        relation_type = str(action.metadata.get("relation_type") or "")
        return relation_type in DEFINITION_RELATION_TYPES or bool(evidence_tags & CONFIRMED_EVIDENCE_ROLE_TAGS)
    ...
    def _is_provisional_family_candidate(
        ...
        return bool(evidence_tags & PROVISIONAL_EVIDENCE_ROLE_TAGS)
    ...
    def _trust_patient_stated_evidence(self, session_id: str) -> int:
        ...
        hypothesis_manager = self.deps.hypothesis_manager
        manager_config = getattr(hypothesis_manager, "config", None)
        expand_top_k = getattr(manager_config, "expand_top_k_hypotheses", 3)
        expand_limit = max(int(expand_top_k or 3), 5)
        ...
        for hypothesis in ranked_hypotheses:
            # 对当前 top hypotheses 逐个回看 R2 期望证据，只要发现和真实病人表述语义对上，就直接补状态。
            try:
    ...
    def _build_verifier_repair_context(
        ...
        # repair_context 是 verifier 拒停和后续“该补什么证据、该偏向哪个候选”的唯一桥梁对象。
    ...
    def _apply_verifier_repair_strategy(self, session_id: str, repair_context: dict) -> None:
        # repair 的第一步不是直接改问，而是先把拒停原因写回候选排序和 tree refresh 标记。
        state = self.deps.state_tracker.get_session(session_id)
    ...
    def _choose_repair_action(
        ...
        # repair 阶段会重新围绕当前答案或强备选构动作池，而不是无条件沿用 search 原 root action。
        state = self.deps.state_tracker.get_session(session_id)
        reject_reason = str(repair_context.get("reject_reason", "missing_key_support"))
        ...
        if current_hypothesis is None:
            return self._choose_next_question_from_search_impl(session_id, search_result)
    ...
    def _select_repair_hypotheses(
        ...
        if reject_reason == "missing_required_anchor":
    ...
    def _score_repair_action(
        ...
        action_role = self._evidence_role_for_action(action)
        missing_roles = {
            item
            for item in self._normalize_string_list(repair_context.get("missing_evidence_roles", []))
            if len(item) > 0
        }
        role_priority = self._repair_action_role_priority(action_role)
        role_gap_bonus = 0.0
        if len(missing_roles) > 0 and action_role in missing_roles:
            role_gap_bonus = 3.5
        elif "disease_specific_anchor" in missing_roles and action_role == "definition_anchor":
            role_gap_bonus = 2.4
        elif "definition_anchor" in missing_roles and action_role == "disease_specific_anchor":
            role_gap_bonus = 2.4
        elif "clear_confirmation" in missing_roles and action_role in {"disease_specific_anchor", "definition_anchor"}:
            role_gap_bonus = 2.0
        ...
        background_role_penalty = 1.25 if action_role == "background_context" else 0.0
    ...
    # 根据当前 anchor 特征推导 repair 最应该补的通用证据角色。
    def _missing_repair_roles_from_anchor_features(self, anchor_features: object) -> list[str]:
        if not isinstance(anchor_features, dict):
        ...
        if anchor_tier == "negative_anchor":
            return ["definition_anchor", "disease_specific_anchor"]
        return []

    # 用 action 的图谱标签、关系和证据 family 判断它能补哪类通用证据角色。
    def _evidence_role_for_action(self, action: MctsAction) -> str:
        label = str(action.target_node_label or action.metadata.get("target_node_label") or "")
        relation_type = str(action.metadata.get("relation_type") or "")
        evidence_tags = {
            item
            for item in self._normalize_string_list(action.metadata.get("evidence_tags", []))
        ...
        if relation_type in {"DIAGNOSED_BY", "CONFIRMED_BY", "DETECTED_BY", "HAS_LAB_FINDING", "HAS_IMAGING_FINDING"}:
            if label == "ClinicalAttribute" or "detail" in evidence_tags or question_type == "detail":
                return "definition_anchor"
            if label in {"LabFinding", "LabTest", "ImagingFinding"}:
                if self._is_background_action_name(normalized_name, evidence_tags):
                    return "background_context"
                return "disease_specific_anchor"

        if relation_type == "REQUIRES_DETAIL" or label == "ClinicalAttribute":
            if self._is_background_action_name(normalized_name, evidence_tags):
                return "background_context"
            return "definition_anchor"

        if relation_type in {"RISK_FACTOR_FOR", "APPLIES_TO"} or label in {"RiskFactor", "PopulationGroup"}:
            return "background_context" if self._is_background_action_name(normalized_name, evidence_tags) else "risk_or_comorbidity"
    ...
    def _maybe_choose_early_exam_context_rescue_action(
        ...
        *,
        ...
        # 这是 root action 之后的一层 override：
        # 如果前几轮信号太弱但当前候选明显 exam-driven，就优先把下一问切回“检查是否做过/结果如何”。
        state = self.deps.state_tracker.get_session(session_id)
        config = self.deps.a3_routing_policy
    ...
    def _choose_low_cost_explorer_action(
        ...
        # low-cost explorer 只在“证据揭示不足”或“检查链路连续落空”时接管，目标是换一条更便宜且可观察的问题。
        state = self.deps.state_tracker.get_session(session_id)
    ...
    def _maybe_choose_low_cost_explorer_action(
        ...
        # 这里保留一个轻薄门控，方便 greedy/no-tree benchmark 一键关闭 explorer 覆盖层。
    ...
    def _should_emit_final_report(
        ...
        # 终止输出的判断优先级是：
        # verifier 明确接受 > 阶段性 stop 且无可继续动作 > 彻底没有搜索信号。
...
class ConsultationBrain:
    ...
    def __init__(self, deps: BrainDependencies) -> None:
        ...
        self.turn_coordinator = TurnCoordinator(self.runtime)
...
def _merge_brain_config(base: dict, overrides: dict | None) -> dict:
    ...
    # 合并策略始终保持“局部覆写”，避免 benchmark 小配置把整段默认配置意外清空。
...
def build_default_brain(
    ...
    # 先加载默认 brain.yaml，再叠加运行期 override，后续所有依赖都从这份兼容配置出发。
    ...
    # 下面开始把兼容 YAML 键逐项映射到运行时组件，确保重构后外部配置语义不回退。
...
def build_default_brain_from_env(
    ...
    # 最常见脚本入口：先从环境读取 Neo4j 连接，再复用默认装配流程构造 ConsultationBrain。
...
def _default_brain_config_path(project_root: Path | None = None) -> Path:
    # 默认配置始终指向仓库内的 configs/brain.yaml，便于脚本和测试共享同一入口。
...
def _read_brain_config_file(path: Path) -> dict:
    # 配置文件缺失或内容不是 dict 时统一降级为空配置，避免启动阶段因 YAML 形态异常直接崩溃。
    if not path.exists():
        return {}
...
def load_brain_config(config_path: str | Path | None = None) -> dict:
    # 配置优先级是：显式传参 > BRAIN_CONFIG_PATH 覆写 > 仓库默认 brain.yaml。
