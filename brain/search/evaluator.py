...
from ..state import FinalAnswerScore, HypothesisScore, PatientContext, ReasoningTrajectory
...
class TrajectoryEvaluatorConfig:
    ...
    discriminative_answer_bonus_weight: float = 0.06
    competitor_suppression_bonus_weight: float = 0.05
...
class TrajectoryEvaluator:
    ...
    def group_by_answer(self, trajectories: Iterable[ReasoningTrajectory]) -> Dict[Tuple[str, str], List[ReasoningTrajectory]]:
        # 从这一层开始，系统不再比较“单条轨迹”，而是比较“最终收敛到同一答案的一组轨迹”。
    ...
    def score_groups(
        ...
        # score_groups 会把 rollout 支持度、路径多样性和 verifier/observed evidence 信号压成最终答案分。
    ...
    def select_best_answer(self, scores: Iterable[FinalAnswerScore]) -> FinalAnswerScore | None:
        # best_answer 是 acceptance、repair 和最终报告共同依赖的单一入口结果。
    ...
    def score_candidate_hypotheses_without_trajectories(
        ...
        *,
        ...
        # 这条 fallback 只在 rollout 基本没产出可用答案组时启用，避免 best_answer 直接断层为空。
    ...
    def _reward_proxy_summary(self, trajectories: Sequence[ReasoningTrajectory]) -> dict:
        ...
        if (
            ...
            and len(alternative_preservation_scores) == 0
        ):
            return {}
    ...
    # 将 rollout 侧的区分性 proxy 转成最终答案层面的轻量 bonus。
    # 这一步仍奖励能把正确答案推到前列的轨迹组，但会显式给“保住高质量 Top-3 覆盖”的组额外加分，
    # 避免 final ranking 只偏好最尖锐的单答案压缩。
    def _final_discriminative_answer_bonus(
        ...
        *,
        ...
        if candidate_rank_position == 1:
            ...
            rank_stability_signal = branch_consistency * 0.78
        elif candidate_rank_position == 3:
            rank_stability_signal = branch_consistency * 0.55
        ...
        bonus = max(
            min(
                ...
                + competitor_bonus
                + support_bonus
                + coverage_stability_bonus
                + rank_stability_bonus,
                0.22,
            ),
            0.0,
        )
