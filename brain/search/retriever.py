...
class GraphRetriever:
    ...
    def retrieve_r1_candidates(
        ...
        # R1 负责把“病人当前说过的阳性线索”变成一批可进入 A2 的候选疾病。
        # R1 的入口特征来自三个来源：
    ...
    def retrieve_observed_anchor_candidates(
        ...
        # 这条 rescue 支路只看真实已确认的强证据，避免 top-k 候选过窄时把真值疾病直接漏掉。
    ...
    def retrieve_scope_sibling_candidates(
        ...
        # sibling rescue 用来召回“同病原/同作用域”的近邻疾病，弥补 R1 只命中一个表层疾病名的情况。
    ...
    def retrieve_r2_expected_evidence(
        ...
        # R2 的输出不是最终问题文本，而是“当前假设最值得继续验证的证据节点列表”。
