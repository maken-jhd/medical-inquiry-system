...
from ..catalog.evidence_family_catalog import GENERIC_FAMILIES, classify_evidence_families
from .io import write_cases_json, write_cases_jsonl
...
class MinimumEvidenceRequirement:
    ...
    # 对应疾病名称。
    disease_name: str
    # 该疾病至少需要覆盖的 evidence family 组列表。
    required_groups: list[set[str]]
    # 按证据大组拆开的最低 family 约束，便于生成时分角色检查。
    required_groups_by_evidence_group: dict[str, list[set[str]]]
    # 该疾病在 catalog 中各 evidence family 的可用数量。
...
class DiseaseAuditRecord:
    ...
    # 疾病图谱标签，通常是 Disease。
    disease_label: str
    # 从审计 JSON 中读取到的标准化证据列表。
    evidence: list[dict[str, Any]]
    # 各证据分组的统计摘要。
    group_summary: dict[str, dict[str, Any]]
    # 单疾病审计报告的顶层 summary 字段。
...
class CompetitionCandidate:
    ...
    # 综合竞争分数，越高表示越适合作为竞争病例。
    competition_score: float
    # 竞争分数各分项来源。
    score_breakdown: dict[str, float]
    # 可作为 shared opening / low-cost overlap 的共享证据。
    shared_low_cost: list[dict[str, Any]]
    # 只对目标病成立、可用于拉开差异的区分性证据。
    target_only_discriminative: list[dict[str, Any]]
    # 只对竞争病成立、可写成目标病阴性的排除证据。
...
class GraphCaseGenerationResult:
    ...
    # 生成过程中的结构化 manifest，记录每个疾病生成/跳过原因。
