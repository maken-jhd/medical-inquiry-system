...
- 病例级 QC 不再只看 family 数量，而是计算 `case_qc_score / case_qc_status / case_qc_reasons`
- QC 会区分 `disease_specific_anchor / definition_anchor / phenotype_support / risk_or_context / background_context`
- `CD4 / HIV / 年龄 / 既往病史 / ART` 等纯背景线索不能单独构成 benchmark 病例
- 感染、代谢/定义、肿瘤、神经类疾病分别按通用核心诊断路径检查，不对单个 smoke 病例写补丁
...
### 7.2 evidence-role case QC

仅满足 minimum evidence family 数量仍然不够。新的病例 QC 会在每个病例生成后计算一份 `case_qc_score`，并写入以下元数据：

- `case_qc_status`：`eligible / weak_anchor / not_benchmark_eligible`
- `case_qc_score`：`0-100` 的病例骨架质量分
- `case_qc_reasons`：降级或不可入 benchmark 的原因
- `case_qc_disease_categories`：该病例被归入的通用诊断路径类型
- `case_qc_positive_role_counts`：阳性槽位按 evidence role 的计数
- `case_qc_missing_core_roles`：缺失的核心诊断路径角色
- `case_qc_high_connectivity_positive_slots`：疑似高连接或背景型阳性证据

QC 的 evidence role 不是按具体疾病名写补丁，而是由证据组、family、关系类型和轻量语义共同推断：

| role | 含义 | 示例 |
| --- | --- | --- |
| `disease_specific_anchor` | 能直接支持某条疾病路径的特异证据 | 病原体、PCR/抗原阳性、特异影像、脑脊液特异检查 |
| `definition_anchor` | 疾病定义或诊断标准型证据 | BMI/血脂/eGFR 阈值、病理/活检、肿瘤标志物 |
| `phenotype_support` | 支持方向但不足以单独定案的表型 | 呼吸、神经、皮肤、消化、眼部症状 |
| `risk_or_context` | 可帮助分层但不是核心锚点的风险/细节 | 暴露史、用药史、病程时间 |
| `background_context` | 高连接背景，不能独立构成病例 | CD4 低、HIV 感染背景、年龄、既往病史、ART |

病例生成后会检查五类通用质量问题：

1. 是否至少有一个 `disease_specific_anchor` 或 `definition_anchor`
2. opening 是否只有 `background_context`
3. `selected_positive_slots` 是否全是高连接或背景证据
4. required family 是否只由泛 family / 背景 family 凑数
5. 是否缺少该疾病主要诊断路径的核心证据

不同疾病路径的核心证据要求如下：

- 感染类：优先要求 `pathogen / disease_specific_lab / serology / imaging` 中至少一类特异证据
- 代谢/定义类：必须有 `metabolic_definition` 相关的 lab/detail/risk 定义证据
- 肿瘤类：必须有 imaging、pathology、tumor marker 或明确肿瘤相关定义证据之一
- 神经类：必须有 neurologic phenotype、CNS imaging 或 CNS lab
- 泛风险背景不能独立构成 benchmark 病例；只含 CD4、HIV、年龄、既往史、ART 的病例会被标记为 `weak_anchor` 或 `not_benchmark_eligible`

为了兼容既有 replay 和统计脚本，生成器仍会保留 `benchmark_qc_status` 字段：

- `case_qc_status = eligible` 时，`benchmark_qc_status = eligible`
- `case_qc_status = weak_anchor / not_benchmark_eligible` 时，`benchmark_qc_status = ineligible`
...
- 从完整病例集中抽取 `20` 个病例，作为快速 replay smoke 输入
- 当前抽样只选择 `case_qc_status=eligible` 的病例
- 抽样优先追求类型均衡；如果某类 eligible 数量不足，会用其他类型 eligible 病例补齐总数
- 输出 `cases.jsonl`、`cases.json`、`manifest.json` 和 `summary.md`

## 14. 当前一轮实际输出

截至当前版本，已经基于 full-evidence catalog 与 evidence-role case QC 重新生成一轮完整图谱病例：

```text
test_outputs/simulator_cases/graph_cases_20260502_role_qc/
```
...
兼容字段 `benchmark_qc_status` 摘要为：

- `eligible = 112`
- `ineligible = 115`

按病例类型拆分的 benchmark eligible 数量为：
...
- `missing_disease_specific_or_definition_anchor = 64`
- `missing_primary_diagnostic_path_core_evidence = 63`
- `opening_background_only = 63`
- `missing_required_family_coverage = 52`
- `all_positive_slots_high_connectivity_or_background = 18`

本轮还生成了新的 `20` 例 smoke 输入：

```text
test_outputs/simulator_cases/graph_cases_20260502_role_qc/smoke20/
```
...
该 smoke20 抽样全部为 `case_qc_status=eligible`，类型分布为：
...
- 只含 CD4、HIV、年龄、既往史等背景线索的病例不再进入 benchmark eligible
- 感染、代谢/定义、肿瘤、神经类病例必须拥有对应核心诊断路径证据，避免用泛 family 数量凑病例质量
...
conda run -n GraduationDesign python scripts/generate_graph_virtual_patients.py \
  ...
  --output-file test_outputs/simulator_cases/graph_cases_20260502_role_qc/cases.jsonl \
  --output-json-file test_outputs/simulator_cases/graph_cases_20260502_role_qc/cases.json \
  --manifest-file test_outputs/simulator_cases/graph_cases_20260502_role_qc/manifest.json \
  --summary-file test_outputs/simulator_cases/graph_cases_20260502_role_qc/summary.md
```
...
conda run -n GraduationDesign python scripts/sample_graph_virtual_patients.py \
  --cases-file test_outputs/simulator_cases/graph_cases_20260502_role_qc/cases.json \
  --output-file test_outputs/simulator_cases/graph_cases_20260502_role_qc/sampled_cases_4x5.json \
  --summary-file test_outputs/simulator_cases/graph_cases_20260502_role_qc/sampled_cases_4x5.md \
...
### 18.3 抽取 role-QC smoke20

```bash
conda run -n GraduationDesign python scripts/build_graph_case_smoke_set.py \
  --cases-file test_outputs/simulator_cases/graph_cases_20260502_role_qc/cases.jsonl \
  --output-root test_outputs/simulator_cases/graph_cases_20260502_role_qc/smoke20 \
  --total-size 20 \
  --target-size-per-type 5 \
  --seed 42
```

这个脚本默认只抽 `case_qc_status=eligible` 的病例。若某个病例类型 eligible 数不足，会保留全部可用 eligible，再用其他类型补齐到总数。
