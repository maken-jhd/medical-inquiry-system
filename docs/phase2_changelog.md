...
## 六十七、2026-05-14：重写第 5 章实验结构，改为主方法与外部基线对比

### 本次目标

- 按导师建议对实验章节进行较大调整
- 暂不在正文中展开消融实验，聚焦本文方法与外部基线的横向比较
...
- 将第 `5` 章标题与结构重写为“实验设置、对比实验、案例分析、本章小结”
- 删除原先围绕开场推理、无树搜索、无修复机制等内部消融展开的主叙事
- 重新组织对比方法，仅保留本文方法（MCTS）、纯大语言模型、文本检索增强生成（TextRAG）和知识图谱检索增强生成（KGRAG）四组结果
- 在数据集说明中明确：主分析仅基于 `benchmark_qc_status=eligible` 的 `112` 例高质量病例
- 重新撰写整体结果分析，重点比较 `top1`、`top3`、接受后精确正确率、错误接受率和平均轮次
- 新增提问行为分析，比较不同方法在症状追问、检查上下文、实验室问题和真值命中问题占比上的差异
- 新增代表性案例分析，以“纵隔淋巴结炎”病例对比本文方法与三类外部基线的提问路径与最终输出

### 结果影响

- 第 `5` 章叙事从“内部机制验证”调整为“主方法与代表性外部方法的性能比较”，更符合当前论文主线
- 实验章节结构更接近常见毕业论文写法，也更贴合导师提出的“实验设置—对比实验—案例分析”组织方式

## 六十八、2026-05-14：整理全量227病例与高质量112病例的对比结果

### 本次目标

- 在实验正文之外，额外整理一份便于查阅的结果目录
- 同时保留全量 `227` 例病例与高质量 `112` 例病例的横向对比结果

### 本次更新

- 新增：
  - [docs/chapter5_comparison_results/README.md](/Users/loki/Workspace/GraduationDesign/docs/chapter5_comparison_results/README.md)
  - [docs/chapter5_comparison_results/full227_comparison.md](/Users/loki/Workspace/GraduationDesign/docs/chapter5_comparison_results/full227_comparison.md)
  - [docs/chapter5_comparison_results/eligible112_comparison.md](/Users/loki/Workspace/GraduationDesign/docs/chapter5_comparison_results/eligible112_comparison.md)
  - [docs/chapter5_comparison_results/comparison_metrics.json](/Users/loki/Workspace/GraduationDesign/docs/chapter5_comparison_results/comparison_metrics.json)

### 具体改动

- 新建 `docs/chapter5_comparison_results/` 目录，集中整理实验章节对比结果
- 在 `full227_comparison.md` 中汇总四种方法在全量 `227` 例病例上的核心指标与简要观察
- 在 `eligible112_comparison.md` 中汇总四种方法在高质量 `112` 例病例上的核心指标与简要观察
- 在 `comparison_metrics.json` 中保存两套结果的结构化指标，便于后续制表或复用
- 在目录说明中写明结果来源路径和两套分析口径的差异

### 结果影响

- 后续撰写论文时，可在主分析之外方便地引用全量病例结果作为补充比较
- 全量结果与高质量结果已被分开整理，能够减少正文与补充材料之间的来回查找成本

## 七十、2026-05-14：补充第5章四类病例分组对比结果与分析

### 本次目标

- 扩充第 `5` 章“对比实验”部分的内容密度
- 在整体结果之外补入四类病例上的分组表现与分析
...
- 在第 `5.2` 节新增“四类病例上的对比表现与分析”小节
- 基于 `eligible` 的 `112` 例高质量病例，按普通问诊型、检查驱动型、竞争鉴别型和低成本问诊型重新整理分组指标
- 新增两张分组对比表，分别展示各方法在四类病例上的命中率、接受可靠性和平均轮次
- 补写分类型文字分析，明确本文方法在普通问诊型与竞争鉴别型上的优势，以及在检查驱动型病例上“高召回、低误接收”的特点
- 同步调整案例分析表号与本章小结，使章节前后叙事保持一致

### 结果影响

- 第 `5` 章“对比实验”部分更加充实，章节结构更接近常见毕业论文写法
- 分组结果能够更具体地说明本文方法在不同病例场景中的优势与局限，增强实验分析的说服力

## 七十一、2026-05-14：调整第5章实验口径为“227例总体比较 + 112例分组分析”

### 本次目标

- 将第 `5` 章整体对比结果从高质量病例切换为全量病例
- 精简提问行为分析，避免过多堆叠问题类型统计
...
- 将第 `5.2.1` 节改写为基于 `227` 例全量病例的整体结果比较，并替换对应表格与分析文字
- 在第 `5.1.2` 节补充说明虚拟病人按照第 `3.3` 节的方法生成，以明确实验对象来源
- 将第 `5.2.2` 节中的“平均症状类问题数、平均检查上下文问题数、平均实验室类问题数”删除
- 保留“真值命中问题占比”，并新增“最低证据组覆盖增量”作为更抽象的过程性行为指标
- 在数据集说明中明确：总体比较使用全量 `227` 例病例，四类病例细分分析仍使用 `112` 例高质量病例

### 结果影响

- 第 `5` 章的总体实验口径与老师希望看到的“全量整体表现”更加一致
- 提问行为分析更聚焦于关键问题命中和关键证据补充能力，避免正文被过细的问题类型统计分散重点

## 七十二、2026-05-14：将第5章四类病例细分比较统一切换为227例全量口径

### 本次目标

- 统一第 `5` 章整体比较与分类型比较的数据口径
- 将四类病例上的对比分析从 `112` 例高质量病例切换为 `227` 例全量病例
...
- 在第 `5.1.2` 节中删除“四类病例分析仍使用 112 例高质量病例”的表述，改为明确说明本章整体与分类型比较均基于 `227` 例全量病例
- 重写第 `5.2.3` 节开头说明，补入四类病例在全量测试集中的样本数
- 用 `227` 例全量病例重新替换四类病例上的 `top1 / top3 / 接受后精确正确率 / 错误接受率 / 平均轮次` 数据
- 重写四类病例的文字分析，删除此前“低成本问诊型仅 1 例”的旧结论，并改为基于全量样本重新解释各类场景表现
- 同步调整本章小结，使其与新的全量分组分析口径保持一致

### 结果影响

- 第 `5` 章整体结果、行为分析和四类病例细分分析现在全部基于统一的 `227` 例全量病例，章节内部口径更加一致
- 四类病例分析的样本基础更充分，尤其低成本问诊型不再依赖单病例现象，实验结论更稳健

## 七十三、2026-05-14：整理第5章案例分析的五个候选病例

### 本次目标

- 为第 `5.3` 节案例分析提供多组可选样例
- 便于在“普通病例、检查驱动型、竞争鉴别型、低成本问诊型”之间挑选更合适的正文案例
...
- 基于四个方法的 replay 结果，筛选出一批 `MCTS` 诊断方向明显优于基线的病例
- 从中进一步整理出 `5` 个适合写入论文正文的候选案例
- 对每个案例补充：
  - 开场描述
  - 四种方法的最终输出与轮次对比
  - `MCTS` 与基线方法的前几轮路径摘要
  - 适合写入正文的原因与可能的不足
- 在文档末尾补充主观推荐顺序，便于后续快速选定最终案例

### 结果影响

- 第 `5.3` 节后续改写时不再需要临时从原始回放结果中翻案例
- 不同类型病例的展示优缺点被提前整理清楚，有助于更快确定最合适的论文案例

## 七十四、2026-05-14：补充多组低成本病例案例候选

### 本次目标

- 扩充第 `5` 章案例候选文档中的低成本病例储备
- 便于后续专门从“低成本问诊型”里挑选正文案例
...
- 在原有 `5` 个主候选之外，新增“低成本病例补充候选”小节
- 追加整理 `4` 个低成本病例：
  - 免疫重建炎症综合征
  - CMV脑炎
  - CMV食管炎或肠炎
  - 猴痘
- 对每个新增病例补充四方法最终输出、前几轮路径亮点、适合用途与注意点

### 结果影响

- 低成本问诊型病例的候选数量明显增加，后续可以按“动作区分度、是否接受、是否适合论文叙事”等不同标准灵活挑选
- 案例候选文档现在既能支持全局挑选，也能支持按病例类型单独挑选

## 七十五、2026-05-14：补充口咽念珠菌病候选的具体问答摘录

### 本次目标

- 让“口咽念珠菌病”案例的差异点展示得更具体
- 方便后续直接将其中的关键问答片段改写进论文正文
...
- 在“候选5：口咽念珠菌病”下新增“关键问答摘录”小节
- 分别补充 `MCTS`、纯大语言模型、`TextRAG`、`KGRAG` 的代表性问答片段
- 重点标出：
  - `MCTS` 如何通过“累及部位 + 口腔局部特征”收敛到口咽念珠菌病
  - 三类基线如何虽然拿到口腔白斑线索，但后续仍持续向食管受累方向误收缩
...
- 调整第 `5` 章案例分析所选病例
- 避免使用“先问是否做过检查”这类容易被理解为后端流程控制的示例
...
- 将案例分析中的代表性病例由“纵隔淋巴结炎”替换为“急性低氧性呼吸衰竭”
- 重写案例分析表格与文字说明，突出 MCTS 首轮直接选择“呼吸频率 > 40 次/分”这一高区分度动作
- 在分析中强调：本文方法的优势并不在于固定先问检查，而在于能够在候选动作中优先选出更有鉴别价值的问题
- 同步改写与纯大语言模型、TextRAG、KGRAG 的提问路径对比，使案例叙事更能体现 MCTS 的方法优势

### 结果影响

- 第 `5` 章案例分析的说服力较此前更强，更能体现 `MCTS` 在动作选择层面的优势
- 案例叙事与正文主张更加一致，避免被误解为简单的后端流程控制效果
